...
def build_main_sheet_bundle(prod_file, sale_file, trans_file, transfer_file):
    ...
    for name, upload, optional in targets:
        ...
        if not frames:
            continue
        sheets[name] = pd.concat(frames, ignore_index=True) if len(frames) > 1 else frames[0]
...
def _is_missing_upload(upload):
    return upload is None or (isinstance(upload, (list, tuple)) and len(upload) == 0)

missing_main = [name for name, up in required_main if _is_missing_upload(up)]
if missing_main:
