...
if overview is None or overview.empty:
    ...
    days_raw = sorted(pd.to_datetime(overview["日期"].dropna().unique()))

    # 可选：将连续两天合并为一天口径再计算总览
    merge_two = st.multiselect(
        "将连续两天合并为一天计算（可选，最多选2天）",
        days_raw,
        format_func=lambda d: pd.to_datetime(d).strftime("%Y-%m-%d")
    )
    overview_use = overview.copy()
    df_lw_use = df_lw.copy()
    if merge_two:
        # 仅取前两个选择，要求连续
        merge_sorted = sorted(pd.to_datetime(merge_two).map(lambda d: pd.to_datetime(d).normalize()).unique())
        if len(merge_sorted) > 2:
            st.warning("请选择最多两天进行合并；已忽略合并。")
        elif len(merge_sorted) == 2:
            delta = (merge_sorted[1] - merge_sorted[0]).days
            if delta > 1:
                st.warning("仅支持合并相邻两天，已忽略合并。")
            else:
                # 合并 overview：两天的量/金额/销量求和，单价按金额/产量重算
                base = overview_use.copy()
                base["_日期_norm"] = pd.to_datetime(base["日期"], errors="coerce").dt.normalize()
                mask_merge = base["_日期_norm"].isin(merge_sorted)
                seg_merge = base.loc[mask_merge].copy()
                rest = base.loc[~mask_merge].drop(columns="_日期_norm", errors="ignore")
                if not seg_merge.empty:
                    agg_cols = {col: "sum" for col in ["产量(kg)", "销量(kg)", "含税金额"] if col in seg_merge.columns}
                    merged = seg_merge.groupby("项目", as_index=False).agg(agg_cols)
                    if "含税金额" in merged.columns and "产量(kg)" in merged.columns:
                        qty = merged["产量(kg)"].replace(0, np.nan)
                        merged["含税单价"] = merged["含税金额"] / qty
                    merged["日期"] = merge_sorted[0]
                    overview_use = pd.concat([rest, merged], ignore_index=True)
                # 合并净重表（毛鸡净重/交鸡量等求和）
                if df_lw_use is not None and not getattr(df_lw_use, "empty", True):
                    try:
                        dcol = next((c for c in ["日期","交鸡日期","记帐日期","记账日期","凭证日期","过账日期"] if c in df_lw_use.columns), None)
                        if dcol:
                            lw_tmp = df_lw_use.copy()
                            lw_tmp["_日期_norm"] = pd.to_datetime(lw_tmp[dcol], errors="coerce").dt.normalize()
                            mask_lw = lw_tmp["_日期_norm"].isin(merge_sorted)
                            if mask_lw.any():
                                keep = lw_tmp.loc[~mask_lw].drop(columns="_日期_norm", errors="ignore")
                                part = lw_tmp.loc[mask_lw].copy()
                                sum_cols = [c for c in part.columns if c not in {dcol, "_日期_norm"}]
                                merged_vals = {}
                                for c in sum_cols:
                                    merged_vals[c] = pd.to_numeric(part[c], errors="coerce").sum(min_count=1)
                                merged_row = {dcol: merge_sorted[0], **merged_vals}
                                df_lw_use = pd.concat([keep, pd.DataFrame([merged_row])], ignore_index=True)
                    except Exception:
                        pass
                st.caption(f"已将 {merge_sorted[0].strftime('%Y-%m-%d')} 和 {merge_sorted[1].strftime('%Y-%m-%d')} 合并为同一天口径。")

    # 采用（可能已合并）后的数据继续后续计算
    overview = overview_use
    df_lw = df_lw_use
    ...
    # 下载所有日期的总览表
    all_days_for_export = sorted(pd.to_datetime(overview["日期"].dropna().unique()))
    export_frames = []
    for _d in all_days_for_export:
        tmp = build_overview_table_for_day(overview, _d, df_lw)
        if tmp is not None and not tmp.empty:
            export_frames.append(tmp)
    if export_frames:
        export_df = pd.concat(export_frames, ignore_index=True)
        export_df = export_df.drop(columns=["日期"], errors="ignore")
        st.download_button(
            "下载所有日期总览（CSV）",
            data=export_df.to_csv(index=False, encoding="utf-8-sig"),
            file_name="总览_所有日期.csv",
            mime="text/csv",
        )
...
if minors is not None and not minors.empty:
    ...
    st.download_button(
        "下载全部子类（CSV）",
        data=export_minors.to_csv(index=False, encoding="utf-8-sig"),
        file_name="子类明细.csv",
        mime="text/csv",
    )
